...
gene   = 41;
populacao       = randn(tampop,gene); % initial population
...
no_geracoes = 500;                   %Inform the number of generations
melhoradp_ant = 0;
estagnacao  = 0;
...
while(1)
    ...
    for i = 1 : tampop
        p = populacao(i,:);
        fav(i) = min(p + 10 * sin(5*p) + 7 * cos(4*p) + sin(p));
    end
